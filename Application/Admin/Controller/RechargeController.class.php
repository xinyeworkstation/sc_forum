...
class RechargeController extends BaseController
{
    ...
    public function index($key = "")
    {
        ...
        $post1 = $model->limit($Page->firstRow . ',' . $Page->listRows)->JOIN('user ON  user.id = recharge.user_id  ')->where($where)->order('recharge.id DESC')->field('recharge.money,recharge.time,recharge.id,user.username')->select();
        //print_r($post1);
